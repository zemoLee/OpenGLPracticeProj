 ...
 * 冷色调滤镜，蓝色通道的颜色加深
 */
public class ColdFilter extends BaseFilter {
    public static String COLD_SHADER = "" +
            ...
            "color.r=max(min(color.r,1.0),0.0);" + //红   传递进来的颜色值，红色部分的值，这里是0--1， 先和1做比较， 超过1的，肯定就不需要了，需要限制一下，不然应该会曝光
    ...
    public ColdFilter(Context mContext) {
        super(mContext);
    }

    public ColdFilter(Context context, String vertexShader, String fragmentShader) {
        super(context, vertexShader, fragmentShader);
    }
    private int changeColorHandle;
    private float[] changeColor=new float[]{0.0f, 0.0f, 0.2f};
    @Override
