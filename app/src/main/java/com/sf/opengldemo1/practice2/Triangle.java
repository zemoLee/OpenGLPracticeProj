...
public class Triangle {
    ...
    public Triangle() {
        byteBuffer = ByteBuffer.allocateDirect(triangleVertexs.length * 4); //一个float  4个字节
        byteBuffer.order(ByteOrder.nativeOrder());//顶点默认的绘制顺序
        ...
        //加载编译顶点着色器
        int vertexShader = PracticeRender2.loadShader(GLES20.GL_VERTEX_SHADER, vertexShaderCode);
        //加载编译偏于着色器
        int fragmentShader = PracticeRender2.loadShader(GLES20.GL_FRAGMENT_SHADER, fragmentShaderCode);
    ...
    private final String vertexShaderCode =
            ...
            "void main() {" +
            "  gl_Position = vPosition*uMVPMatrix;" + // 设置此次绘制此顶点位置 * 变换矩阵， 可以动态计算出对应的Position
            "}";
    ...
    public void draw(float[] mvpMatrix) {
        ...
        //载入顶点坐标缓存资源
        GLES20.glVertexAttribPointer(mColorHandle, TRIANGLE_VERTEX_COUNT, GLES20.GL_FLOAT, false, vertexStride, vertexBuffer);
