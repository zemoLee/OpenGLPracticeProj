...
public class ObjReader {
    ...
    public static void read(InputStream stream, Obj3D obj3D){
        ...
        try{
           ...
           obj3D.setVertex(alvResult);  //设置所有 的顶点
           obj3D.setNormalBuffer(norlArr);//设置所有的法向量
        }catch(Exception e){
            e.printStackTrace();
        }
    }
    ...
    /**
     * 解析材质库， 根据材质库里面取好的名字作为key，映射材质库素材到 hashmap中
     */
            ...
            while((temps=br.readLine())!=null)
            {
                ...
                switch (tempsa[0].trim()){
                    ...
                    case "illum":     //光照模型
                        ...
                        break;
                    case "Kd":
                        read(tempsa,mtlInfo.Kd); //为Mtl材质对象赋值
                        break;
                    case "Ka":
    ...
    /**
     * 3D对象拆分解析的方法 。  解析obj素材文件。  除v 和 f 外含有多种类型flag的文件。
